...
const usePagination = ({ count, onPageChange, pageSize = DEFAULT_PAGE_SIZE }) => {
  const pagesCount = useMemo(() => (count ? getPagesCount(count, pageSize) : 1), [count, pageSize]);
  return {
    ...
    pagesCount,
    pageNumbers: useMemo(
      () => Array.from(Array(pagesCount || 1).keys(), (item) => item + 1),
      [pagesCount],
    ),
  };
};
